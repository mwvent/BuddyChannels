...
class SendMessageTask extends Task {
    ...
    \BuddyChannels\Message $message, \BuddyChannels\Main $plugin) {
        $this->plugin = $plugin;
    ...
    public function onRun($currenttick) {
        ...
        if (is_null($message->serverid)) {
            //
            $this->plugin->externalFormatUserMessage($message, true);
            $this->messageFormatter->formatUserMessage($message);
            $this->plugin->externalFormatUserMessage($message, false);
            //
            $this->plugin->externalFromatForChannels($message, true);
            $this->messageFormatter->formatForChannels($message);
            $this->plugin->externalFromatForChannels($message, false);
        } else {
            $this->messageFormatter->formatForChannels($message);
        }
        ...
        if (!is_null($message->message_receivers_lcase_usernames)) {
            ...
            if (!$sent) {
                ...
                $message->sender->sendMessage(
                        Main::translateColors("&", "&f&l&oYOU ---> " . implode(",", $message->message_receivers_lcase_usernames) . "&r&f : " . $message->msg));
            }
        }
