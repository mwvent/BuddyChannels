...
class Database {
	...
	$db_setup_queries["insert this server entry into buddychannels servers table"] = "
	 ...
	 foreach($db_setup_queries as $query_name => $query_sql) {
		...
		if($qresult === false) {
		    $this->criticalError("Database set-up error executing "  . $query_name . " " . 
					  $this->db_statements[$query_name]->error );
		}
	...
	if( $this->db_statements[$queryname] === false ) {
	    $this->criticalError("Database error preparing query for  "  . $queryname . 
				 ": " . $this->db->error);
	    return false;
	}
	return true;
	...
	$result = $this->db_statements[$thisQueryName]->bind_param(
	    ...
	    $message->username,
	    $message->userrank,
	    $message->senderChannel_number,
	    $message->senderChannel_name,
	    $message->msg, 
	...
	$result =  $this->db_statements[$thisQueryName]->bind_result(
	    $messagetime, $serverid, $servername, $username, $user_rank,
	    $channel, $channelname, $message, $unfiltered_message,
	    $was_baby, $was_shouting, $was_blocked
	);
	if( $result === false ) {
	    $this->criticalError("Failed to bind result " . $thisQueryName . ": " . $this->db_statements[$thisQueryName]->error);
	    return array();
	...
	while( $this->db_statements[$thisQueryName]->fetch() ) {
	    $this->lastForeignMessageTime = $messagetime;
	    $newmessage = new \BuddyChannels\ForeignMessage (
		$serverid, $servername, $username, $user_rank, $channel, $channelname, $message, $was_shouting);
	...
	// report channel info
	$msg = "&dYou are currentley in channel &c" .  $playerChannelNumber . " &o&n&6{" . 
		$this->read_cached_channelNames($playerChannelNumber) . "}";
	$player->sendMessage(Main::translateColors("&", $msg));
